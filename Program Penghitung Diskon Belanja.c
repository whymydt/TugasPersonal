...
int main() {
  int error, start = 1, input, validCheck, valid, countTransaksi = 0, diskon,
             totalBelanja = 0, totalDiskon = 0, loyalty = 0, totalLoyalty = 0;
  char pilihan[10], inputChar[13];
  printf("\033[0;32m====================================================\n");
  printf("\t\tPROGRAM PENGHITUNG DISKON BELANJA\n");
  printf("====================================================\033[0m\n");
  printf("Selamat datang di Program Penghitung Diskon Belanja!\n");
  while (start == 1) {
    strcpy(pilihan, "1");
    while (strcmp(pilihan, "1") == 0) {
      ...
      while (validCheck == 1) {
        ...
        while (valid != strlen(inputChar)) {
          printf("\033[0;31mNilai tidak valid, input nilai belanja dengan "
                 "benar!\033[0m\n");
          valid = 0;
          break;
        }
      ...
      } else if (input >= 1000000 && countTransaksi % 4 != 0) {
        diskon = input * 0.3;
        loyalty = 0;
        printf("\033[0;32mSelamat! Anda mendapat diskon 30%%!\033[0m\n");
      } else if (input < 200000 && countTransaksi % 4 == 0) {
        loyalty = 0.2 * input;
        printf("\033[0;32mSelamat! Anda mendapatkan diskon loyalty sebesar "
               "20%%!\033[0m\n");
      } else if (input >= 200000 && input < 500000 && countTransaksi % 4 == 0) {
        diskon = input * 0.1;
        loyalty = 0.2 * (input - diskon);
        printf("\033[0;32mSelamat! Anda mendapatkan diskon belanja sebesar "
               "10%% dan diskon loyalty sebesar 20%%!\033[0m\n");
      } else if (input >= 500000 && input < 1000000 &&
                 countTransaksi % 4 == 0) {
        diskon = input * 0.2;
        loyalty = 0.2 * (input - diskon);
        printf("\033[0;32mSelamat! Anda mendapatkan diskon belanja sebesar "
               "20%% dan diskon loyalty sebesar 20%%!\033[0m\n");
      } else if (input >= 1000000 && countTransaksi % 4 == 0) {
        diskon = input * 0.3;
        loyalty = 0.2 * (input - diskon);
        printf("\033[0;32mSelamat! Anda mendapatkan diskon belanja sebesar "
               "30%% dan diskon loyalty sebesar 20%%!\033[0m\n");
      } else {
        break;
      }
      totalDiskon += diskon;
      ...
      printf("\033[0;36mApakah anda ingin menambah transaksi? (1 Ya/2 Tidak) : "
             "\033[0m");
      scanf("%s", &pilihan);
      getchar();
    }
    ...
    if (strcmp(pilihan, "2") == 0) {
      ...
      printf("\nTotal pembayaran\t  : Rp. %d",
             totalBelanja - totalDiskon - totalLoyalty);
      start = 0;
    }
  }
  if (start == 0) {
